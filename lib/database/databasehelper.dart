...
class DatabaseHelper {
  ...
  Future<Database> get database async {
    if (_database == null) _database = await initialeDatabase();
    return _database!;
  }
  ...
  void _createDB(Database db, int version) async {
    await db.execute(
        'CREATE TABLE ${table_name}(${note_id} INTEGER PRIMARY KEY  AUTOINCREMENT,${note_name} TEXT,${note_email} TEXT,${note_desc} TEXT)');
  }
  ...
  Future<int> updateNote(int id, Note note) async {
    ...
    var result = db.rawUpdate(
        'UPDATE ${table_name} SET ${note_name}=?,${note_email}=?,${note_desc}=? WHERE ${note_id}=?',
        [note.name, note.email, note.description, id]);
    return result;
  }
  ...
  Future<int> getNoteCount() async {
    ...
    List<Map<String, dynamic>> result =
        await db.query('SELECT 8 FROM ${table_name}');
